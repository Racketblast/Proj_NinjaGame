...
UENUM(BlueprintType)
enum class ETargetMission : uint8
{
	Patrol,
	Camera, 
	Electrical,
 ...
 */
UCLASS()
class PROJ_NINJAGAME_API ATargetAIController : public AAIController
{
	GENERATED_BODY()
	public:
	ATargetAIController();
	...
	ETargetState GetCurrentState() const { return CurrentState; }

	ETargetMission GetCurrentMission() const { return CurrentMission; }
	void SetCurrentMission(ETargetMission NewMission) { CurrentMission = NewMission; }

	void StartChasingFromExternalOrder(FVector LastSpottedPlayerLocation);

	//För missions
	void AssignMission(ETargetMission NewMission, FVector MissionLocation);
...
protected:
	...
	ETargetState CurrentState = ETargetState::Patrolling;
	...
	ETargetMission CurrentMission = ETargetMission::Patrol;
