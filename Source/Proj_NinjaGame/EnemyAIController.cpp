...
void AEnemyAIController::OnPossess(APawn* InPawn)
{
	...
	if (ControlledEnemy)
	{
		...
		ControlledEnemy->OnSuspiciousLocation.AddDynamic(this, &AEnemyAIController::HandleSuspiciousLocation);

		//UE_LOG(LogTemp, Warning, TEXT("AEnemyAIController OnPossess"));
	...
	if (CurrentState == EEnemyState::Patrolling)
	{
		if (Result.IsSuccess())
		{
			...
			if (PatrolPoints.Num() > 1)
			{
				...
				TWeakObjectPtr<AEnemyAIController> WeakThis(this); // Använde detta för att stoppa en tidigare krasch 
				GetWorldTimerManager().SetTimer(WaitHandle, [WeakThis]()
				{
					if (!WeakThis.IsValid()) return;
					AEnemyAIController* Self = WeakThis.Get();
...
void AEnemyAIController::RefreshChaseTarget()
{
...
void AEnemyAIController::BeginSearch()
{
	...
	{
		GetWorldTimerManager().SetTimer(EndSearchTimerHandle, this, &AEnemyAIController::EndSearch, ControlledEnemy->GetSearchTime(), false);
	});
}
...
void AEnemyAIController::LookAround()
{
	if (!ControlledEnemy || !IsValid(ControlledEnemy)) return;
	if (GetWorld()->bIsTearingDown) return;
	//UE_LOG(LogTemp, Warning, TEXT("AEnemyAIController LookAround 1"));
	...
	//UE_LOG(LogTemp, Warning, TEXT("AEnemyAIController LookAround"));
...
void AEnemyAIController::OnHeardSound(FVector SoundLocation)
{
	if (!ControlledEnemy) return;
...
void AEnemyAIController::HandleSuspiciousLocation(FVector Location)
{
...
void AEnemyAIController::StartSmoothRotationTowards(const FVector& TargetLocation, float RotationSpeed)
{
	if (!ControlledEnemy) return;
...
void AEnemyAIController::OnUnPossess()
{
	if (ControlledEnemy)
	{
		ControlledEnemy->OnSuspiciousLocation.RemoveDynamic(this, &AEnemyAIController::HandleSuspiciousLocation);
...
void AEnemyAIController::RunChaseFailsafe(float DeltaSeconds)
{
	if (CurrentState != EEnemyState::Chasing) return;
...
//Mission system
void AEnemyAIController::AssignMission(EEnemyMission NewMission, FVector MissionLocation)
{
...
void AEnemyAIController::CompleteMission()
{
...
void AEnemyAIController::StartMissionMoveTo(FVector Location)
{
    if (!IsValid(this) || !ControlledEnemy || !IsValid(ControlledEnemy)) return;
...
//Stun
void AEnemyAIController::StunEnemy(float Duration, TOptional<EEnemyState> WantedState)
{
	if (!IsValid(this) || !ControlledEnemy || !IsValid(ControlledEnemy)) return;
...
void AEnemyAIController::RetryMoveToNextPatrolPoint()
{
	StopMovement();
	ControlledEnemy->GetCharacterMovement()->MaxWalkSpeed = ControlledEnemy->GetWalkSpeed();
	MoveToNextPatrolPoint();
